...
# выгрузки лежат в той же папке, что и app.py, и подхватываются автоматически —
# поэтому папка с данными это просто папка со скриптом (в репозитории они рядом)
DATA_FOLDER = HERE
...
@st.cache_data(show_spinner=False)
def load_months(paths, _mtimes=None):
    ...
    ни имён, ни констант в коде править не нужно. _mtimes передаётся отдельным
    аргументом специально для кэша: если файл результата пересобрали заново
    (build_result.py перезаписал его), время изменения меняется, и кэш
    Streamlit сам сбрасывается — без этого он продолжал бы показывать
    старые цифры под тем же именем файла.
    """
    out, broken = {}, []
    for p in paths:
        try:
            df, meta = load(p, Path(p).stat().st_mtime)
...
def read_decisions():
    ...
    try:
        pay_df = pd.read_excel(DECISIONS_FILE, sheet_name='Выплаты')
        con_df = pd.read_excel(DECISIONS_FILE, sheet_name='Договоры')
...
files = sorted(str(p) for p in DATA_FOLDER.glob(DATA_GLOB) if not p.name.startswith('~$')) if DATA_FOLDER.exists() else []
if not files:
    masthead()
    st.error(f'Положите выгрузки вида **{DATA_GLOB}** в папку **{DATA_FOLDER}** и обновите страницу.')
    st.stop()

MONTHS_DATA, broken = load_months(tuple(files), tuple(Path(p).stat().st_mtime for p in files))
if not MONTHS_DATA:
    masthead()
...
# ─────────────────────────── период и фильтры ───────────────────────────
with st.sidebar:
    st.markdown(f'<div style="display:flex;align-items:center;gap:.5rem;margin-bottom:1rem">'
